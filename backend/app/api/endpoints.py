...
def login(login_data: schemas.UserLogin):
    try:
        ...
        access_id = login_data.access_id or f"MOCK-ID-{role.upper()}"
        if not role:
            role = "fan"
...
def update_incident_status(incident_id: int, update: schemas.IncidentUpdate, db: Session = Depends(get_db)):
    try:
...
def update_volunteer_task(volunteer_id: int, task: str, status: str = "busy", db: Session = Depends(get_db)):
    try:
...
def update_gate(gate_id: int, update: schemas.GateUpdate, db: Session = Depends(get_db)):
    try:
...
def update_food_stall(stall_id: int, update: schemas.FoodStallUpdate, db: Session = Depends(get_db)):
    try:
...
def create_food_stall(stall: schemas.FoodStallCreate, db: Session = Depends(get_db)):
    try:
...
def update_transport_route(route_id: int, update: schemas.TransportRouteUpdate, db: Session = Depends(get_db)):
    try:
...
def update_match(match_id: int, update: schemas.MatchUpdate, db: Session = Depends(get_db)):
    try:
